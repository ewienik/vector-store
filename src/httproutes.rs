...
#[utoipa::path(
    get,
    path = "/api/v1/indexes/{keyspace}/{table}/size",
    ...
    responses(
        (status = 200, description = "Index size", body = usize)
    )
)]
async fn get_index_size(
    ...
    match index.size().await {
        Err(err) => {
            let msg = format!("index.size request error: {err}");
            error!("get_index_size: {msg}");
            (StatusCode::INTERNAL_SERVER_ERROR, msg).into_response()
        }

        Ok(size) => (StatusCode::OK, response::Json(size)).into_response(),
    }
}
    ...
    match index.ann(request.embeddings, request.limit).await {
        ...
        Ok((primary_keys, distances)) => {
            if primary_keys.len() != distances.len() {
                let msg = format!(
                    "wrong dimensions of an ann response: number of primary_keys = {}, number of distances = {}",
                    primary_keys.len(),
                    distances.len()
                );
                error!("post_index_ann: {msg}");
                        ...
                        let primary_keys: anyhow::Result<_> = primary_keys
                            ...
                            .map(|primary_key| {
                                if primary_key.0.len() != primary_key_columns.len() {
                                    bail!(
                                        "wrong dimensions of a primary key: {}, {}",
