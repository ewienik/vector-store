...
pub(crate) struct Index {
    ...
    pub(crate) target_name: ColumnName,
...
impl DbBasic {
    ...
    pub(crate) fn add_index(
        ...
        let Some(keyspace) = db.keyspaces.get_mut(keyspace_name) else {
            bail!("a keyspace {keyspace_name} not exists");
        };
        let Some(table) = keyspace.tables.get(&index.table_name) else {
            bail!("a table {} not exista", index.table_name);
        };
        if !table.embeddings.contains_key(&index.target_name) {
            bail!(
                "a table {} not contains a target column {}",
                index.table_name,
                index.target_name
            );
        }
    ...
    pub(crate) fn del_index(
        ...
        if keyspace.indexes.remove(index_name).is_none() {
            bail!("an index {index_name} not exists");
        }
    ...
    pub(crate) fn insert_values(
        ...
        let Some(table) = keyspace.tables.get_mut(table_name) else {
            bail!("a table {table_name} not exists");
        };
        let Some(column) = table.embeddings.get_mut(target_name) else {
                ...
                .flat_map(|(keyspace_name, keyspace)| {
                    keyspace
                        ...
                        .map(|(index_name, index)| DbCustomIndex {
                            ...
                            target: index.index.target_name.clone(),
                        })
                })
                .collect()))
        ...
        DbIndex::GetProcessedIds { tx } => tx
            .send(Ok(stream::iter(
                db.0.read()
                    ...
                    .and_then(|table| table.embeddings.get(&metadata.target_name))
                    .map(|rows| {
                        rows.iter()
        ...
        DbIndex::ResetItems { keys } => {
            if let Some(rows) =
                db.0.write()
                    ...
                    .and_then(|table| table.embeddings.get_mut(&metadata.target_name))
            {
